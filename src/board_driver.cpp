...
bool BoardDriver::loadCalibration() {
  if (!ChessUtils::ensureNvsInitialized()) {
    WebSerial.println("NVS init failed - calibration not loaded");
    return false;
  }
  Preferences prefs;
  ...
  prefs.end();
  WebSerial.println("Board calibration loaded from NVS");
  return true;
}

void BoardDriver::saveCalibration() {
  if (!ChessUtils::ensureNvsInitialized()) {
    WebSerial.println("NVS init failed - calibration not saved");
    return;
  }
  Preferences prefs;
  prefs.begin("boardCal", false);
  ...
  prefs.end();
  WebSerial.println("Board calibration saved to NVS");
}
  ...
  while (true) {
    ...
    if (pressedCount == 0) {
      ...
      if (now - lastWarningTime >= CALIBRATION_WARNING_INTERVAL_MS) {
        lastWarningTime = now;
        WebSerial.printf("Board not empty - %d sensor(s) still detecting a magnet:\n", pressedCount);
        for (int row = 0; row < NUM_ROWS; row++)
          for (int col = 0; col < NUM_COLS; col++)
            if (rawState[row][col])
              WebSerial.printf("  GPIO %d + 74HC595 Q%c (pin %d)\n", hwConfig.rowPins[row], shiftRegOutput(col), shiftRegPin(col));
      }
    }
    ...
    if (count == 1) {
      if (foundRow == lastRow && foundCol == lastCol) {
        ...
        if (lastRow >= 0 && lastCol >= 0) {
          WebSerial.println("Sensor reading unstable - detected square changed. Hold piece steady on one square.");
          WebSerial.printf("  Previous: GPIO %d + 74HC595 Q%c (pin %d), Current: GPIO %d + 74HC595 Q%c (pin %d)\n", hwConfig.rowPins[lastRow], shiftRegOutput(lastCol), shiftRegPin(lastCol), hwConfig.rowPins[foundRow], shiftRegOutput(foundCol), shiftRegPin(foundCol));
        }
        lastRow = foundRow;
        lastCol = foundCol;
        stableStart = 0;
      ...
      if (now - lastWarningTime >= CALIBRATION_WARNING_INTERVAL_MS) {
        lastWarningTime = now;
        if (count == 0) {
          WebSerial.println("No sensor detecting a magnet - place a piece on the requested square");
        } else {
          WebSerial.printf("Multiple sensors (%d) detected simultaneously but need exactly 1:\n", count);
          for (int row = 0; row < NUM_ROWS; row++)
            for (int col = 0; col < NUM_COLS; col++)
              if (rawState[row][col])
                WebSerial.printf("  GPIO %d + 74HC595 Q%c (pin %d)\n", hwConfig.rowPins[row], shiftRegOutput(col), shiftRegPin(col));
...
bool BoardDriver::calibrateAxis(Axis axis, uint8_t* axisPinsOrder, size_t NUM_PINS, bool firstAxisSwapped) {
  if ((NUM_ROWS != NUM_COLS) || (NUM_ROWS != NUM_PINS)) {
    WebSerial.println("Non-square boards not supported for calibration");
    return false;
  }
  ...
  for (int i = 0; i < NUM_PINS; i++) {
    ...
    if (axis == ColsAxis && expectedRawPin != -1) {
      ...
      if (actualPin != expectedRawPin) {
        if (useRow)
          WebSerial.printf("[ERROR] Expected piece on rank 1 = row %d (GPIO %d) but detected on row %d (GPIO %d) which is not rank 1. Place piece on %s.\n", expectedRawPin, hwConfig.rowPins[expectedRawPin], actualPin, hwConfig.rowPins[actualPin], square);
        else
          WebSerial.printf("[ERROR] Expected piece on rank 1 = col %d (74HC595 Q%c, pin %d) but detected on col %d (74HC595 Q%c, pin %d) which is not rank 1. Place piece on %s.\n", expectedRawPin, shiftRegOutput(expectedRawPin), shiftRegPin(expectedRawPin), actualPin, shiftRegOutput(actualPin), shiftRegPin(actualPin), square);
    ...
    if (i == 0) {
      firstRow = row;
      firstCol = col;
      WebSerial.println("Remove the piece");
      waitForBoardEmpty();
      continue;
    }
    ...
    if (detectedAxis == UnknownAxis) {
      ...
      WebSerial.printf("Ambiguous %s calibration (no orientation detected). Retry.\n", axisToChessRankFile(axis).c_str());
      showCalibrationError();
      i = -1;
      continue;
    }
    ...
    if (counts[pin] > 0) {
      ...
      if (detectedAxis == RowsAxis)
        WebSerial.printf("[ERROR] Row %d (GPIO %d) already has %s assigned. Retry %s.\n", pin, hwConfig.rowPins[pin], assignedRankFile, square);
      else
        WebSerial.printf("[ERROR] Col %d (74HC595 Q%c, pin %d) already has %s assigned. Retry %s.\n", pin, shiftRegOutput(pin), shiftRegPin(pin), assignedRankFile, square);
      showCalibrationError();
      i--;
      continue;
    }
...
bool BoardDriver::runCalibration() {
  ...
  WebSerial.println("========================== Board calibration required ==========================");
  WebSerial.println("- Connect to the AP and open the web interface to configure GPIO pins if needed");
  WebSerial.println("- Type 'skip' within 5 seconds to temporarily skip calibration (reboot to calibrate later)");
  WebSerial.println("  LEDs and sensors won't have correct mapping until calibration is completed");
  ...
  while (millis() - startTime < 5000) {
    if (Serial.available()) {
      ...
      input.toLowerCase();
      if (input == "skip") {
        WebSerial.println("[SKIP] Calibration skipped - using default mapping");
        WebSerial.println("[SKIP] Sensors/LEDs will NOT work correctly!");
        WebSerial.println("[SKIP] You will be asked to calibrate again on next reboot");
        ...
        WebSerial.println("Unknown command \"" + input + "\" Type \"skip\" to skip calibration or wait 5 seconds for calibration to begin");
      }
    }
    delay(50);
  }
  WebSerial.println("");
  WebSerial.println("- Empty the board to begin calibration - instructions will follow once an empty board is detected");
  WebSerial.println("- WARNING: Low GPIO voltage can cause unreliable shift register behavior (74HC595 needs Vih > 0.7*Vcc) use a level shifter or HCT variant");
  WebSerial.println("- WARNING: Shift register outputs shouldn't power 8 sensors directly from 1 output pin, use transistors! (max 35mA per pin but each A3144 draws ~10mA");
  WebSerial.println("- WARNING: If powering multiple sensors from one shift register pin, expect voltage drop and shift register failure");
  WebSerial.println("- TIP: Try both magnet sides and move magnet closer if sensor doesn't trigger");
  WebSerial.println("================================================================================");
  waitForBoardEmpty();
  ...
  if (swapAxes1 != swapAxes2) {
    WebSerial.println("Inconsistent axis orientation detected during calibration. Restarting calibration.");
    showCalibrationError();
  ...
  WebSerial.println("LED mapping calibration:");
  ...
  for (int pixelIndex = 0; pixelIndex < LED_COUNT; pixelIndex++) {
    ...
    WebSerial.println("Place a piece on the white LED");
    waitForSingleRawPress(row, col);

    uint8_t logicalRow = toLogicalRow[swapAxes ? col : row];
    uint8_t logicalCol = toLogicalCol[swapAxes ? row : col];
    if (logicalUsed[logicalRow][logicalCol]) {
      WebSerial.printf("Duplicate square %c%c detected. Retry LED %d.\n", (char)('a' + logicalCol), (char)('8' - logicalRow), pixelIndex);
      showCalibrationError();
      pixelIndex--;
      continue;
    }
...
void BoardDriver::loadLedSettings() {
  if (!ChessUtils::ensureNvsInitialized()) {
    WebSerial.println("NVS init failed - LED settings not loaded");
    return;
  }
  Preferences prefs;
  ...
  prefs.end();
  WebSerial.printf("LED settings loaded: brightness=%d, dimMultiplier=%d\n", brightness, dimMultiplier);
}

void BoardDriver::saveLedSettings() {
  if (!ChessUtils::ensureNvsInitialized()) {
    WebSerial.println("NVS init failed - LED settings not saved");
    return;
  }
  Preferences prefs;
  prefs.begin("ledSettings", false);
  ...
  prefs.end();
  WebSerial.printf("LED settings saved: brightness=%d, dimMultiplier=%d\n", brightness, dimMultiplier);
}

void BoardDriver::triggerCalibration() {
  // Mark calibration as needed by clearing the saved calibration
  if (!ChessUtils::ensureNvsInitialized()) {
    WebSerial.println("NVS init failed - cannot trigger calibration");
    return;
  }
  Preferences prefs;
  prefs.begin("boardCal", false);
  prefs.clear();
  prefs.end();
  WebSerial.println("Board calibration cleared - rebooting ...");
  ESP.restart();
}
...
void BoardDriver::loadHardwareConfig() {
  ...
  if (!prefs.isKey("ver")) {
    prefs.end();
    WebSerial.println("No saved hardware config, using compile-time defaults");
    return;
  }
...
void BoardDriver::saveHardwareConfig(const HardwareConfig& config) {
  ...
  WebSerial.println("Hardware config saved to NVS");
}